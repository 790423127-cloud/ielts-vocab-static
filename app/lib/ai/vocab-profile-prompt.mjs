export const AI_VOCAB_PROMPT_VERSION = "ielts-g-main-plus-synonym-meanings-v6";
...
export function buildAiWordProfilePrompt(inputItems) {
  ...
  return [
    ...
    "7. Return 0-4 reliable synonyms that can replace the headword in its primary sense. For every synonym, synonym_details must contain the same word, its part of speech, and a concise Chinese meaning. If existing_synonyms is non-empty, keep exactly those supplied terms and only complete their details; do not replace, add, or remove them. Use empty arrays when no safe replacement exists. Never return the headword itself, capitalization/spacing/hyphen/apostrophe variants, or British/American spelling variants (for example airmail/air mail and encyclopaedia/encyclopedia); never add merely related words.",
