...
// These pairs were produced by earlier suffix/substring matching. They are
// lexical lookalikes, not grammatical forms of the owner word.
const REVIEWED_FALSE_FORM_LINKS = new Set([
  "canva::canvas",
  "ear::earring",
  "sacrifice::sacred",
  "even::evening",
...
function isStoredInflectedReference(word) {
  return word?.entryType === "inflected-form" && isInflectedReferenceWord(word);
}
...
export function normalizeFormList(value) {
  ...
  value.forEach((item) => {
    ...
    const word = String(item && typeof item === "object" ? item.word || "" : item || "").trim();
    if (!word) return;
...
export function buildLocalFormFamilyResult(sourceWords) {
  const stats = {
    ...
    selfLinksRemoved: 0,
    danglingFormsRemoved: 0,
    nonInflectedReferenceLinksRemoved: 0,
  ...
  const words = sourceWords.map((word) => {
    ...
    const forms = normalizeFormList(sourceForms);
    const wordFamily = normalizeFamilyList(sourceFamily);
  ...
  // Search/spelling aliases are redirects, not grammatical forms. Only an
  // explicit inflected-form record may be written into a base card's forms.
  const references = words.filter(isStoredInflectedReference);
  ...
  words.forEach((owner) => {
    ...
    owner.forms.forEach((form) => {
      ...
      if (REVIEWED_FALSE_FORM_LINKS.has(`${ownerKey}::${formKey}`)) {
      ...
      const target = idTarget || wordTarget;
      if (!target) {
        // A grammatical form may be display-only: pure inflections are merged
        // into their lemma and intentionally do not need another brushable
        // card. Keep only forms whose spelling is actually generated by this
        // owner; a sibling such as disqualified -> disqualifying still fails
        // the check and is removed.
        if (classifySurfaceInflection(owner.word, form.word)) nextForms.push(form);
        else stats.danglingFormsRemoved += 1;
        return;
      }

      if (isInflectedReferenceWord(target) && !isStoredInflectedReference(target)) {
        stats.nonInflectedReferenceLinksRemoved += 1;
        return;
      }

      if (!isStoredInflectedReference(target)) {
        nextForms.push(form);
        return;
      }
...
export function buildLocalOptimizeResult(sourceWords) {
  ...
  return {
    ...
    stats: {
      ...
      danglingFormsRemoved: formResult.stats.danglingFormsRemoved,
      nonInflectedReferenceLinksRemoved: formResult.stats.nonInflectedReferenceLinksRemoved,
...
function inferMergedFormChineseType({ baseWord = "", formWord = "", pos = "" } = {}) {
  const kind = classifySurfaceInflection(baseWord, formWord);
  const posText = String(pos || "").toLowerCase();
  const isNoun = /\bnoun\b|名词/u.test(posText);
  const isVerb = /\bverb\b|动词/u.test(posText);
  const isAdjective = /\badjective\b|\badj\b|形容词/u.test(posText);

  if (kind === "plural") return "复数形式";
  if (kind === "plural-or-third-person") {
    if (isNoun && !isVerb) return "复数形式";
    if (isVerb && !isNoun) return "第三人称单数";
  }
  if (kind === "past-or-past-participle" && isVerb) return "过去式 / 过去分词";
  if (kind === "present-participle" && isVerb) return "现在分词 / 动名词";
  if (kind === "comparative" && isAdjective) return "比较级";
  if (kind === "superlative" && isAdjective) return "最高级";
  if (kind === "possessive" && isNoun) return "所有格";
  return "";
}

export function getFormChineseType(type = "", context = {}) {
  const lower = String(type || "").toLowerCase();

  if (lower.includes("base-form") || lower.includes("base form")) {
    return "原形";
  }
  ...
  if (lower.includes("third-person")) {
    return "第三人称单数";
  }
  ...
  if (lower.includes("present participle") || lower.includes("gerund")) {
    return "现在分词 / 动名词";
  }

  if (lower.includes("comparative")) return "比较级";
  if (lower.includes("superlative")) return "最高级";
  if (lower.includes("possessive")) return "所有格";
  if (
    lower.includes("merged-form")
    || lower.includes("corpus-observed-form")
    || lower === "form"
  ) {
    return inferMergedFormChineseType(context) || "相关词形";
  }

  return type || "变形";
}

export function getFormExplanation(baseWord, meaning, form, basePos = "") {
  ...
  if (/base[ -]form/i.test(String(form?.type || ""))) return "";
...
export function getFormHint(form, context = {}) {
  const typeCn = getFormChineseType(form?.type, context);
