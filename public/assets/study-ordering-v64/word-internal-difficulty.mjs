...
export function wordInternalDifficultyScore(word) {
  const rawExplicit = word?.studyDifficultyScore;
  const explicit = rawExplicit === null || rawExplicit === undefined || rawExplicit === ""
    ? Number.NaN
    : Number(rawExplicit);
...
export function createWordInternalDifficultyProfile(words) {
  ...
  // Scores are clamped integers in [0, 100]. A fixed histogram produces the
  // same sorted score sequence without sorting thousands of wrapper objects.
  // Tier assignment only depends on the primary score, not the tie order.
  const scoreCounts = new Uint32Array(101);
  scored.forEach((item) => {
    scoreCounts[Math.max(0, Math.min(100, item.score))] += 1;
  });
  const scores = [];
  ...
  scoreCounts.forEach((count, score) => {
    if (!count) return;
    uniqueScoreCount += 1;
    for (let occurrence = 0; occurrence < count; occurrence += 1) scores.push(score);
  });
  ...
  const scoreByWord = Object.create(null);
  scored.forEach((item) => {
    if (!item.key) return;
    // The old score sort made the lowest-scored duplicate win. Preserve that
    // behavior without sorting all scored word objects.
    if (scoreByWord[item.key] == null || item.score < scoreByWord[item.key]) {
      scoreByWord[item.key] = item.score;
      tierByWord[item.key] = item.score <= assignment.easierMax
        ? WORD_STUDY_DIFFICULTY_TIER.EASIER
        : item.score >= assignment.harderMin
          ? WORD_STUDY_DIFFICULTY_TIER.HARDER
          : WORD_STUDY_DIFFICULTY_TIER.STANDARD;
    }
  });
