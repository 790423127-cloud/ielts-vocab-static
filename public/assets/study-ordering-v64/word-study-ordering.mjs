...
export const WORD_STUDY_ORDER_SNAPSHOT_VERSION = 5;
...
function groupSortValue(group) {
  ...
  for (const entry of group) result = Math.min(result, entry.order);
  return Number.isFinite(result) ? result : 0;
}

function sortGroupsBySourceOrder(groups) {
  return groups
    .map((group) => ({ group, order: groupSortValue(group) }))
...
function orderSceneEntries(entries, preferredScene = "") {
  ...
  const globalBucket = {
    positions: profiles.map((profile) => profile.position),
    cursor: 0
  };
  const sceneBuckets = new Map();
  profiles.forEach((profile) => {
    if (!profile.scene) return;
    if (!sceneBuckets.has(profile.scene)) {
      sceneBuckets.set(profile.scene, { positions: [], cursor: 0 });
    }
    sceneBuckets.get(profile.scene).positions.push(profile.position);
  });
  sceneBuckets.forEach((bucket) => {
    bucket.positions.sort((left, right) => (
      profiles[left].entry.order - profiles[right].entry.order
    ));
  });
  const lowestRemaining = (bucket) => {
    if (!bucket) return null;
    while (
      bucket.cursor < bucket.positions.length
      && !remaining.has(bucket.positions[bucket.cursor])
    ) {
      bucket.cursor += 1;
    }
    return bucket.cursor < bucket.positions.length
      ? bucket.positions[bucket.cursor]
      : null;
  };
  const ordered = [];
  let currentPosition = lowestRemaining(globalBucket);
  let previousPosition = null;

  while (remaining.size) {
    if (!remaining.has(currentPosition)) {
      // Prefer continuing inside the active scene before jumping globally.
      const activeScene = Number.isInteger(previousPosition)
        ? profiles[previousPosition].scene
        : preferredScene;
      currentPosition = lowestRemaining(sceneBuckets.get(activeScene))
        ?? lowestRemaining(globalBucket);
    }
    ...
    let strongestPosition = null;
    let strongestScore = -1;
    scores.forEach((score, candidatePosition) => {
      if (score < 96) return;
      if (
        score > strongestScore
        || (
          score === strongestScore
          && profiles[candidatePosition].entry.order
            < profiles[strongestPosition]?.entry.order
        )
      ) {
        strongestPosition = candidatePosition;
        strongestScore = score;
      }
    });
    if (Number.isInteger(strongestPosition)) {
      currentPosition = strongestPosition;
      continue;
    }

    // Soft break: stay in-scene when possible instead of jumping to global min order.
    currentPosition = lowestRemaining(sceneBuckets.get(current.scene))
      ?? lowestRemaining(globalBucket);
  }
...
function buildAssociationGroups(entries) {
  ...
  return [...sceneBuckets.entries()]
    ...
    .concat(
      sortGroupsBySourceOrder(standalone)
...
export function orderStudyWordIndices(indices, pool, {
  ...
  // The overwhelmingly common path is the physical lexicon order with no
  // relative-difficulty filter. Its output is already `source`; resolving
  // every word into wrapper objects and then creating one array per word only
  // adds a large allocation/GC spike on the 10k+ main lexicon.
  if (
    normalizedMode === WORD_STUDY_ORDER_MODE.CURRENT
    && normalizedDifficultyMode === WORD_STUDY_DIFFICULTY_MODE.DEFAULT
  ) {
    return source;
  }
  ...
  if (normalizedMode === WORD_STUDY_ORDER_MODE.FAMILY) {
    groups = sortGroupsBySourceOrder(
      groupFamilyEntries(entries).map(sortFamilyMembers)
    );
...
export function reconcileWordStudyOrderSnapshot(
  ...
  const compatibleSnapshot = Number(snapshot?.version) === WORD_STUDY_ORDER_SNAPSHOT_VERSION
  ...
  // (numeric indices in the pool shift, keys do not). A snapshot from an older
  // ordering algorithm must not keep its historical sequence, though: use the
  // freshly generated fallback order and only preserve its cursor in that case.
  if (compatibleSnapshot && stableKeys.length) {
    stableKeys.forEach((key) => {
      appendIndex(eligibleByKey.get(key));
    });
  } else if (compatibleSnapshot) {
